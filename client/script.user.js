...
// @author       Igloo-Garage (Rex) / Fixed by Assistant
...
(function() {
    ...
    const LANGUAGE = localStorage.getItem('yt_assistant_lang') || 'zh';
    ...
    // ✅ 修复点：改用普通字符串拼接，彻底解决 'Unterminated template' 报错
    ...
    function createUI() {
        ...
        const resetBtn = createEl('div', { id: 'yt-panel-reset', title: '重置选项' }, ['🔄']);
        ...
        el('dl-btn-action').onclick = () => {
            if (isDownloading) {
                ...
                el('dl-btn-action').textContent = "🛑 正在强制刹车...";
                el('dl-btn-action').style.background = "gray";

                GM_xmlhttpRequest({
                    method: "POST",
                    url: LOCAL_SERVER + "/abort",
                    onload: () => {
                        isDownloading = false;
                        el('dl-status').textContent = "⚠️ 任务已杀停";
                        el('dl-btn-action').textContent = "🚫 已中止";
                        setTimeout(() => {
                            el('dl-btn-action').disabled = false;
                            el('dl-btn-action').textContent = T.btnAction;
                            el('dl-btn-action').style.background = "";
                        }, 2000);
                    }
                });
                return;
            ...
            GM_xmlhttpRequest({
                ...
                onload: (res) => {
                    ...
                    try {
                        ...
                        if (json.status === "aborted") {
                            el('dl-status').textContent = "⚠️ 任务被中断";
                            return;
                        }
                    } catch(e) {}

                    el('dl-btn-action').textContent = "✅ 全部搞定";
                ...
                onerror: () => {
                    isDownloading = false;
                    el('dl-btn-action').disabled = false;
                    el('dl-btn-action').textContent = "❌ 失败";
                }
            });
        };
